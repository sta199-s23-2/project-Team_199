...
olympics_new |> 
  filter(Sport == "Swimming", medalyesno == "yes", Year > 1999) |>
...
olympics_new |> 
  ...
  ggplot(aes(x = Team)) + 
    geom_bar() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(title = "Amount of Medals Recieved per Country", x = "Country", y = "Number of Medals")
...
olympics_new |> 
  ...
  ggplot(aes(x = Age, y = medal)) + 
    geom_boxplot() + 
  facet_wrap(~Sex) + labs(title = "Male and Female Age as it Relates to Medals Earned",
